...
_coaches: dict[str, FitnessCoach] = {}  # video_stem -> coach


def _get_coach(video_stem: str, exercise: str = "auto") -> FitnessCoach:
    with _coach_lock:
        if video_stem not in _coaches:
            _coaches[video_stem] = FitnessCoach(exercise=exercise)
        return _coaches[video_stem]


def _record_chunk_metrics(video_stem: str, frames: int, detections: list, latency_ms: float, coach: dict | None = None):
...
async def stream_chunk(
    ...
    # ── Pose + Fitness coach (lightweight: last frame only) ───────────
    coach_payload = {}
    pose_payload = {"ok": False}
    try:
        coach = _get_coach(video_stem)
        coach.set_exercise((exercise or "auto").strip().lower())

        if frame_files and _pose_estimator.available:
            import cv2

            last_fp = frame_files[-1]
            img = cv2.imread(last_fp)
            if img is not None:
                pose_frame = _pose_estimator.infer_bgr(img)
                pose_payload = {
                    "ok": pose_frame.ok,
                    "angles": pose_frame.angles,
                    "meta": pose_frame.meta,
                }
                if pose_frame.ok:
                    coach_payload = coach.update(pose_frame.angles)
                else:
                    coach_payload = coach.update({})
        else:
            # still keep state stable
            coach_payload = coach.update({})
            if not _pose_estimator.available:
                pose_payload = {"ok": False, "reason": "mediapipe_not_installed"}
                coach_payload["cue"] = "Pose coaching is disabled on the server. Install 'mediapipe' and restart to enable rep counting."
                coach_payload["score"] = 0.3
            else:
                pose_payload = {"ok": False, "reason": "no_frames"}
    except Exception as e:
        pose_payload = {"ok": False, "reason": f"pose_error: {e}"}
    ...
    _record_chunk_metrics(video_stem, frames_count, chunk_detections, elapsed_ms, coach=coach_payload)
    ...
    return {
        ...
        "exercise": (exercise or "auto").strip().lower(),
